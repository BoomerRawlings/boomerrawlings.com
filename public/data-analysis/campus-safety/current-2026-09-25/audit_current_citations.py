...
check('2024 report availability matches qualified current populations and calculated rates', resident_2024==14 and f"{coverage['available_rates']['2024']['residents']} combined criminal-offense rates are available among {resident_2024} institutions" in campus_text)
...
check('Visible overview binds qualified 2024 combined-housing coverage to current data and four citations',all(text in overview for text in ["For 2024, {coverage.available_rates['2024'].residents} of the 42 schools",'rate combining the listed housing offenses','they do not measure a student’s chance','Housing is included in the wider on-campus total']) and overview.count('data-campus-citation')==4 and all(url in overview for url in ['${revision}/rates.csv','https://bjs.ojp.gov/content/pub/pdf/ccsvsftr.pdf#page=131','${revision}/coverage.json','https://www.ecfr.gov/current/title-34/subtitle-B/chapter-VI/part-668/subpart-D/section-668.46']))
...
expected_adopted={('243744',2024):14203,('243744',2025):14042,('211440',2022):3458,('211440',2023):3764,('211440',2024):3988,('243744',2023):14137,('130794',2022):6255,('130794',2023):6064,('130794',2024):6011,('130794',2025):6082}
check('Ten added resident records preserve exact independently supported values',set(adopted)==set(expected_adopted) and all(int(adopted[k]['value'])==v for k,v in expected_adopted.items()))
...
check('Every earlier expansion population source hash matches the recovered source bytes',all(adopted[k]['source_sha256']==sha(p) for k,p in [(('243744',2024),stanford2024),(('243744',2025),stanford2025),(('211440',2022),cmu2024),(('211440',2023),cmu2026),(('211440',2024),cmu2026)]))
...
GAP=CAMPUS/'research/resident_gap_completion_2026_09_26'
yale_pdf=GAP/'ivy/yale_w023_housing.pdf'
stanford2023_pdf=GAP/'private/sources/stanford/facts2024-archived-0e7d8215.pdf'
yale_text=pdf_text(yale_pdf)
check('Yale original student housing totals, fall basis and update date',all(v in yale_text for v in ['6,255','6,064','6,011','6,082','Counts as of fall','10-16-2025']) and all(adopted[('130794',y)]['source_sha256']==sha(yale_pdf) for y in [2022,2023,2024,2025]))
check('Stanford2023 archived original actual student fields and autumn date',all(v in pdf_text(stanford2023_pdf,48) for v in ['autumn quarter 2023','7,207 students','6,930 students']) and 7207+6930==int(adopted[('243744',2023)]['value']) and adopted[('243744',2023)]['source_sha256']==sha(stanford2023_pdf))
qualified=load(DATA/'resident_evidence.json')
check('Qualified housing observations have sources and never claim rate eligibility',len(qualified['observations'])==37 and len({r['unitid'] for r in qualified['observations']})==14 and all(r['eligible_for_rate'] is False and r['reason_no_rate'] and r['source_url'].startswith('https://') for r in qualified['observations']))
check('Qualified evidence is a separate displayed ledger, not adopted data',load(SITE/'src/data/campus-resident-evidence.json')==qualified and all((r['unitid'],y) not in adopted for r in qualified['observations'] for y in r['years']))
check('Crime and housing source-review dates distinguished','Crime-source review: 25 September 2026. Housing-source review: 26 September 2026.' in (SITE/'src/components/CampusReader.astro').read_text(encoding='utf8'))
for target in ['ETh-cgP0JKxIjKWO0cCYYbMBWWRbAovpNlmyYUWeK9GRkA','Stanford-FactBook2024_WEB.pdf','resident_evidence.json']:
 check('New report clickable source: '+target,any(target in x['url'] for x in campus_links))
...
('C09','Resident denominators','State Auditor documents actual2022–2024 fall occupancy for ten UCs and SDSU; ten added Stanford/CMU/Yale observations extend the reviewed population series. Housing properties are not fully matched to Clery geography.','State Auditor PDF62–65; Stanford Facts2025 PDF48 and current housing page; CMU2024 PDF84 and2026 PDF81; exact source fields and sums reread.','PASS_WITH_QUALIFICATION'),
...
('C43','Yale student populations','Four fall snapshots2022–2025 are actual student headcounts, not family-member counts. They produce no2025 rate without complete crime counts.','Official OIR W023 PDF1–3; independent visual extraction; qualified property alignment retained.','PASS_WITH_QUALIFICATION'),
('C44','Stanford2023 historical recovery','Autumn2023 university-provided student residents14,137, from7,207 students in undergraduate housing plus6,930 in graduate housing.','Original university PDF captured14April2024; archive digest matches; PDF48/printed46 visually reviewed.','PASS_WITH_QUALIFICATION'),
('C45','Qualified housing evidence','37 observations across14 other institutions distinguish partial, approximate and unresolved counts. None enters a rate; academic/fiscal spans do not imply fall censuses.','Separate resident_evidence.json source-by-source review, reader isolation checks and explicit source/date/scope reasons.','PASS_WITH_QUALIFICATION'),
...
reviewed += [DATA/'resident_evidence.json',SITE/'src/data/campus-resident-evidence.json',HERE/'RESIDENT_GAP_AMENDMENT.md']
source_versions=[yale_pdf,stanford2023_pdf,sdsu_current,sdsu_prior,ucsd,ucsc,occupancy,stanford2024,stanford2025,cmu2024,cmu2026,cmu_eligibility,ut2024,EXPANSION/'ivy_northeast/raw/princeton_asr_2026.pdf',EXPANSION/'ivy_northeast/raw/harvard_asr_report.pdf',EXPANSION/'ivy_northeast/raw/harvard_fire_2025.pdf']
...
corrections=[
 ...
 'The opening coverage finding now reads from the current audited2024 combined-housing-rate count:14of42. It retains four direct citations and does not imply a national coverage estimate or safety ranking.',
 'Ten qualified student-housing observations added for Stanford/Carnegie Mellon/Yale; Texas spring residents withheld because student-only scope is not established. Previously blocked report files recovered, Princeton2026 added, and exact sources/periods exposed per selection.',
