...
requireCheck('2024 criminal-total housing availability matches overview',data.institutions.filter(i=>readerResult(i).rate!==null).length===10&&coverage.available_rates['2024'].residents===10);
...
requireCheck('Initial school source marker derives from shared official-name ordering',sdsuNumber===18&&component.includes('[S{sourceNumber}]')&&!component.includes('[S24]'));
...
const reviewed=['src/components/CampusReader.astro','src/components/CampusReaderSources.astro','src/lib/campus-reader.js','src/data/campus-reader-copy.js','src/data/campus-school-context.json'];
