...
For the **2024 combined criminal-offense housing comparison**, 33 of 42 institutions have usable counts; 14 have adopted resident populations; all 14 have a paired rate. The 26 September housing-source review adds Yale in 2022–2025 and Stanford in 2023, supplementing the five earlier additions. Yale and Stanford supply two adopted 2025 resident populations, but neither has a complete current institutional 2025 crime count. No 2025 enrollment denominator is adopted. Missing values are not zeros, and an unverified denominator does not establish that public housing figures do not exist.

`RESIDENT_GAP_AMENDMENT.md` is the current scientific-change record. **`EXPANSION_AMENDMENT.md`, `PRESENTATION_AMENDMENT.md` and `RELEASE_PACKAGE_CHECK.json` are historical records:** their older coverage, replay and campus Portable Document Format (PDF) hashes identify preceding releases. They do not verify this revision. `EXPANSION_REPLAY_AUDIT.json` identifies the clean replay and packaged-text privacy check. The final ZIP checksum is listed in the public artifact manifest; an archive cannot contain its own final checksum.
...
- `population_sources.csv`: every adopted enrollment/resident observation, source URL/page/hash, reference period and scope qualification. Ten additions supplement the retained populations.
- `resident_evidence.json`: 37 qualified observations across 14 further institutions. These retain actual partial counts, approximations and unresolved definitions, each with a source and reason no rate is calculated. They never enter the denominator ledger. Academic/fiscal spans are explicitly labeled; a display-year match is not adoption as a fall census.
...
`RESIDENT_GAP_AMENDMENT.md` describes the 26 September additions and the distinction between complete, partial, approximate and unverified populations. `EXPANSION_AMENDMENT.md` preserves the preceding 25 September release; its old coverage and report checksum are historical.

## Population and count qualifications

The 11 California resident series retain the State Auditor's actual fall occupancy. Carnegie Mellon adds university and separately listed fraternity/sorority housing census counts; its housing office excludes graduate students from on-campus housing. The table is labeled Pittsburgh, although the 2024 total matches university-wide undergraduate enrollment. Stanford adds actual student residents in undergraduate and graduate housing. Its autumn 2023 count comes from the archived original university fact book; its 2024 university-authored factbook was recovered from a publisher mirror after the official file disappeared, and identical original official bytes cannot be authenticated. Its 2025 counts come from the current official page. Yale's complete student-housing census retains its fall dates and property-alignment qualification.
...
python expansion/resident-gap/compile_resident_evidence.py --output replay
```

This reconstructs aggregate counts, populations, exclusions, geography and rates from preserved inputs, including `expansion/`. It does not redownload sources or verify later versions. Compare the **ten scientific products** with the packaged files: `source_cells.csv`, `superseded_source_cells.csv`, `population_sources.csv`, `dataset.json`, `rates.csv`, `geography.json`, `case_study.json`, `coverage.json`, `source_inventory.json` and `build_inputs.json`. The third command separately reconstructs `resident_evidence.json`; its qualified observations never enter the scientific builder. See the identified checksums in `EXPANSION_REPLAY_AUDIT.json`. The old `RELEASE_PACKAGE_CHECK.json` is not this expanded replay result.
